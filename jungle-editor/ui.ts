...
class JungleUI {
    ...
    static renameProject(id: string): void {
    ...
    static renderProjectsDashboard(): void {
        ...
        projects.forEach(p => {
            ...
            `;
            (card.querySelector('.rename-proj-btn') as HTMLElement).onclick = (e) => {
                e.stopPropagation();
                this.renameProject(p.id);
            };
            (card.querySelector('.delete-proj-btn') as HTMLElement).onclick = (e) => {
                e.stopPropagation();
    ...
    static switchToFile(filename: string): void {
        const p = this.getCurrentProject();
        if (!p) return;
    ...
    static updateCodeHighlight(): void {
        ...
        function addOpsAndPunct(s: string): string {
    ...
    static _highlightJS(escaped: string, tok: (cls: string, content: string) => string): string {
    ...
    static _highlightCSS(escaped: string, tok: (cls: string, content: string) => string): string {
        return escaped.replace(
    ...
    static updateLineNumbers(): void {
        const count = editor.value.split('\n').length;
        lineGutter.textContent = Array.from({length: count}, (_, i) => i + 1).join('\n') + '\n';
    }
    static updateLinesOfCodeCount(): void {
        const code = editor.value;
        locDisplay.textContent = `LOC: ${code.split('\n').length}`;
    }
    static showCustomModal({ title, placeholder, onConfirm, description = "" }: ModalOptions): void {
        const modal = document.getElementById('custom-modal') as HTMLElement;
        const mTitle = document.getElementById('modal-title') as HTMLElement;
        const mInput = document.getElementById('modal-input') as HTMLInputElement;
        const mCancel = document.getElementById('modal-cancel') as HTMLElement;
        const mConfirm = document.getElementById('modal-confirm') as HTMLElement;
        const mBodyText = document.getElementById('modal-body-text') as HTMLElement;
