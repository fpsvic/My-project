...
def _match(q: str, *keywords: str) -> bool:
...
def _derivative_response(expr: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Symbolic Differentiation.\n- **Method:** Polynomial Power Rule mapping."
...
def _integral_response(expr: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Symbolic Integration.\n- **Method:** Reverse Power Rule."
    terms = _parse_polynomial_terms(expr)
    if not terms:
        body = (
    ...
    for coeff, power in terms:
        ...
        val_str = str(int(raw_val)) if raw_val == int(raw_val) else f"\\frac{{{coeff}}}{{{new_power}}}"
...
def _quadratic_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Solve quadratic ax² + bx + c = 0.\n- **Method:** Quadratic Formula."
    abc_m = re.search(
...
def _linear_equation_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Solve linear equation ax + b = c.\n- **Method:** Algebraic isolation."
    m = re.search(
...
def _statistics_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Descriptive Statistics.\n- **Method:** Direct computation on dataset."
    ...
    q = query.lower()
    if _match(q, "std", "standard deviation"):
    ...
    rows = [
        ...
        ("Median", f"${median:.4g}$"),
        ("Mode", f"${mode_str}$"),
        ("Variance (σ²)", f"${variance:.4g}$"),
        ("Std Deviation (σ)", f"${std_dev:.4g}$"),
    ]
    table = "| Statistic | Value |\n|-----------|-------|\n" + "\n".join(
        f"| {label} | {val} |" for label, val in rows
    )
    body = (
...
def _combinations_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Count arrangements.\n- **Method:** nCr / nPr formula."
...
def _geometry_response(query: str, mode: str) -> str:
    ...
    if "circle" in q:
        ...
        body = (
            ...
            + _table([("Area", "$\\pi r^2$", f"${area:.4g}$"), ("Circumference", "$2\\pi r$", f"${perim:.4g}$")])
            + f"\n\n$$A = \\pi ({r})^2 = {area:.4g}$$"
    ...
    if "rectangle" in q or "square" in q:
        ...
        body = (
            ...
            + _table([("Area", "$l \\times w$", f"${area:.4g}$"), ("Perimeter", "$2(l + w)$", f"${perim:.4g}$")])
            + f"\n\n$$A = {l} \\times {w} = {area:.4g}$$"
    ...
    if "sphere" in q:
        ...
        body = (
            ...
            + _table([("Volume", "$\\frac{4}{3}\\pi r^3$", f"${vol:.4g}$"), ("Surface Area", "$4\\pi r^2$", f"${sa:.4g}$")])
            + f"\n\n$$V = \\frac{{4}}{{3}}\\pi ({r})^3 = {vol:.4g}$$"
    ...
    if "cylinder" in q:
        if len(nums) >= 2:
            ...
            body = (
                ...
                + _table([("Volume", "$\\pi r^2 h$", f"${vol:.4g}$"), ("Surface Area", "$2\\pi r(r+h)$", f"${sa:.4g}$")])
                + f"\n\n$$V = \\pi ({r})^2 ({h}) = {vol:.4g}$$"
    ...
    if "cone" in q:
        if len(nums) >= 2:
            ...
            body = (
                ...
                + _table([
                    ("Volume", "$\\frac{1}{3}\\pi r^2 h$", f"${vol:.4g}$"),
                    ("Surface Area", "$\\pi r(r+l)$", f"${sa:.4g}$"),
                    ("Slant Height", "$\\sqrt{r^2+h^2}$", f"${slant:.4g}$"),
                ])
                + f"\n\n$$V = \\frac{{1}}{{3}}\\pi ({r})^2({h}) = {vol:.4g}$$"
                f"\n\n[MATH_CARD: formula: cone r={r} h={h} | result: vol={vol:.4g} | style: violet]"
            )
            return _thinking_wrap(thinking, heading, body, mode)

    if "cube" in q and nums:
        s = nums[0]
        vol = s ** 3
        sa = 6 * s ** 2
        body = (
            f"**Cube** with side $s = {s}$:\n\n"
            + _table([("Volume", "$s^3$", f"${vol:.4g}$"), ("Surface Area", "$6s^2$", f"${sa:.4g}$")])
            + f"\n\n$$V = ({s})^3 = {vol:.4g}$$"
...
def _pythagorean_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Solve right triangle sides.\n- **Method:** a² + b² = c²."
    nums = _parse_number_list(query)
    q = query.lower()
...
def _prime_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Primality test.\n- **Method:** Trial division."
    nums = _parse_number_list(query)
    if not nums:
...
def _gcd_lcm_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Greatest Common Divisor / Least Common Multiple.\n- **Method:** Euclidean algorithm."
    ...
    q = query.lower()
    result_label = f"LCM={lcm}" if _match(q, "lcm", "least common") else f"GCD={gcd}"
...
def _fibonacci_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Generate Fibonacci numbers.\n- **Method:** Iterative computation."
    nums = _parse_number_list(query)
    n = min(int(nums[0]) if nums else 10, 50)
...
def _factorial_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Compute factorial.\n- **Method:** n! = n × (n-1) × … × 1."
    nums = _parse_number_list(query)
    if not nums:
...
def _arithmetic_response(query: str, mode: str) -> str:
    ...
    thinking = "- **Intent:** Arithmetic Reduction.\n- **Method:** Safe expression evaluation."
    q = query.lower().strip()
    ...
    if re.search(r"(\d+)\s*(?:factorial|!)", q):
...
def generate_math_response(query: str, mode: str) -> str:
    ...
    if _match(q, "integral", "integrate", "antiderivative"):
